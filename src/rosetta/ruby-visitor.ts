...
import { AstRenderer, CommentSyntax } from 'jsii-rosetta/lib/renderer';
import { SubmoduleReference } from 'jsii-rosetta/lib/submodule-reference';
import { stripCommentMarkers, voidExpressionString, matchAst, nodeOfType } from 'jsii-rosetta/lib/typescript/ast-utils';
import { ImportStatement } from 'jsii-rosetta/lib/typescript/imports';
import {
  isEnumAccess,
...
export class RubyVisitor extends DefaultVisitor<RubyLanguageContext> {
  ...
  // External languages are registered by name; 'ruby' is not a member of
  // the upstream TargetLanguage enum, so cast to satisfy the base-class
  // contract (the enum is string-valued and every consumer keys on the
  // string).
  public readonly language = 'ruby' as TargetLanguage;
   ...
   */
  // NOTE: upstream's renderer does not yet dispatch PostfixUnaryExpression /
  // ConditionalExpression to typed handlers (they fall through to raw-text
  // best-effort). These implementations become live when that dispatch
  // lands upstream — a small standalone PR extracted from the fork.
  public postfixUnaryExpression(node: ts.PostfixUnaryExpression, context: RubyVisitorContext): OTree {
   ...
   */
  public conditionalExpression(node: ts.ConditionalExpression, context: RubyVisitorContext): OTree {
    return new OTree([
   ...
   */
  public arrowFunction(node: ts.ArrowFunction, context: RubyVisitorContext): OTree {
    return this.renderLambda(node, node.body, context);
  }

  /** Translates `function (a) { ... }` expressions exactly like arrows. */
  public functionExpression(node: ts.FunctionExpression, context: RubyVisitorContext): OTree {
    return this.renderLambda(node, node.body, context);
  }
  ...
  private renderLambda(
    ...
    if (!simple) {
      // Fork rosetta had AstRenderer.renderUnsupported for this; replicate its
      // best-effort contract with upstream primitives.
